...
def knot_move(head_cord, knot_cord):
    ...
    ((low_idx, low_coord), (high_idx, high_coord)) = sorted(
        enumerate(relative_head_cord), key=lambda x: abs(x[1])
    )
    movement = [0, 0]
    if abs(high_coord) == 2:
        if abs(low_coord) >= 1:
...
def part_n(k_notes: int):
    with open("day09.txt", "r") as file:
        movements = chain.from_iterable(
            ((line.split()[0]) * int(line.split()[1]) for line in file.readlines())
        )
